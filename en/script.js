...
class LanguageManager {
  ...
  setupLanguageSwitchers() {
    ...
    const mobileLangItems = document.querySelectorAll('.mobile-lang-item');
  ...
  saveLanguagePreference(lang) {
    localStorage.setItem('devory-language', lang);
  }
  ...
  handleLanguageChange(lang) {
    ...
    this.updateActiveStates(lang);
    
    // If we're not on the correct language path, redirect
    if (lang === 'en' && !currentPath.startsWith('/en')) {
      this.redirectToLanguage('en');
    } else if (lang === 'de' && currentPath.startsWith('/en')) {
      this.redirectToLanguage('de');
    }
  }
